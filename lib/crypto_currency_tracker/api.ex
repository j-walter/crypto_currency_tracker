  ...
  def get_currency(currency_id, user_details) when is_nil(currency_id) or currency_id in @digital_currencies do
    if !!currency_id  do
        ...
        prices = Enum.reduce(@digital_currencies, %{}, fn currency_id, acc ->
          Map.put(Map.put(acc, currency_id, ApiAgent.get(currency_id)), :alerts, Map.merge(Map.get(acc, :alerts, %{}), %{currency_id => Map.delete(Alert.client_view(Alert.get(currency_id, user_details || %User{}) || %Alert{}), :digital_currency)}))
        end)
